...
from policy_network_PPO import Network
...
while True:
    ...
    while not episode_done:
        ...
        old_network_actions = up_probability
        ...
        tup = (observation_delta, action_dict[action], reward, old_network_actions)
    ...
    if episode_n % args.batch_size_episodes == 0:
        states, actions, rewards, old_network_actions = zip(*batch_state_action_reward_tuples)
        rewards = discount_rewards(rewards, args.discount_factor)
        rewards -= np.mean(rewards)
        rewards /= np.std(rewards)
        batch_state_action_reward_tuples = list(zip(states, actions, rewards,old_network_actions))
